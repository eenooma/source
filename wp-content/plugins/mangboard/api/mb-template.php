		...
		if($mb_user_pid==mbw_get_comment_item("fn_user_pid") || intval(mbw_get_board_option("fn_modify_level")) <= $mb_user_level){
			$modify_html		= '<div class="cmt-input-box">';
				$modify_html		.= '<div class="cmt-input-head">';
					$modify_html		.= '<form name="'.mbw_get_param("board_name").'_form_comment_modify" id="'.mbw_get_param("board_name").'_form_comment_modify" method="post" action="">';
		...
		if(strpos(mbw_get_param("list_type"),'calendar')===0){
			...
			if($list_data["total_count"] > 0){
				...
				foreach($board_items as $item){	
					...
					$list_html					.= '<div class="gallery-item-box '.$category_item_class.' '.mbw_get_param("responsive_class").'" style=""><div class="gallery-item-wrap">';
			...
			if($list_data["total_count"] > 0){
				...
				foreach(mbw_get_board_items() as $item){
					...
					$list_html			.= '<tr id="'.mbw_get_id_prefix()."tr_".$list_index.'"'.$item_class.'>';
					foreach($list_model as $data){
						if(mbw_check_item($data)){
				...
				$list_html			.= '<tr><td colspan="'.$list_data["cols"].'" align="center" style="text-align:center;">'.__MM("MSG_LIST_ITEM_EMPTY").'</td></tr>';
			}
			$board_data["head"]					= "";
			$board_data["body"]					= $list_html;
			$board_data["foot"]						= "";
