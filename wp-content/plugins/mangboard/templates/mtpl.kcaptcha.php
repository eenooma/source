...
if(!function_exists('mbw_get_kcaptcha_template')){
	function mbw_get_kcaptcha_template($mode, $data){
		...
		if($item_type=='kcaptcha_img'){
			...
			$template_start	.= '<input'.$data["ext"].__STYLE("width:".$data["width"].";height:".$data["height"]." !important;".$data["style"]).' name="'.mbw_set_form_name($data["type"]).'" maxlength="6" value="'.$data["value"].'"'.$t_id.' type="text"'.$t_title.' />';
		}		
